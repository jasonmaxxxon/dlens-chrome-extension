...
- PR #25 merged: `RECONCILE` started with tested stale-result ignore for Compare/Product/Folder/PR UI async response writes plus a narrow session-scoped snapshot guard.
...
- SEAM_GUARD zero-bypass closure: the 14 legacy `entrypoints/background.ts` write bypasses now route through seam-owned helpers for compare cache, Product context/cache, global snapshots, snapshot payloads, and tab cleanup. `npm run storage:seam-guard` reports 0 allowlisted bypasses, so `SEAM_GUARD` is now 🟩.
- PR #40 merged: locks session queue/refresh terminal-stale snapshot lanes so stale queue/refresh responses skip storage writes and `state/updated` broadcasts.
...
- API/JOBS work-truth closure (2026-06-16): backend `/worker/status` now exposes backlog + retry + expired-running + analysis terminal summary; extension projects via `projectBackendWorkStatus()` into a single `BackendWorkUiState`; `reconcileSessionItem` promotes failed analysis into `lastErrorKind="analysis_failed"`; `classifyInitialReadError` separates `backend_unavailable` / `route_error` / `version_mismatch` / `lag_tolerated` / `ok`; ProcessingStrip and TopicProcessingStatus surface recovery copy (incl. "重啟處理" for expired-running); a five-case shared negative-fixture set locks both layers with exact case-name set equality. `API` / `JOBS` move from 🟡 to 🟢; not 🟩 yet because no live-failure gate exists for visible recovery surfaces.
