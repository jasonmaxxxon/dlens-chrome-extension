...
- Split growing popup/background orchestration before adding digest/watch-mode work. `entrypoints/background.ts` is now 3802 lines and `src/ui/useInPageCollectorAppState.ts` is now 2542 lines.
- Pipeline spine status: PR #21 merged the typed trace event stream, PR #22 merged requestId trace correlation, PR #23 merged terminal VM `ui.ready`, PR #24 merged the typed trace summarizer + live harness gate, PR #25 started `RECONCILE` UI stale-result ignore, PR #28 added the first committed `ui.ready` fixture CI gate, and PR #26 guards background snapshot writes for `session/refresh-all` plus `session/queue-items-and-start-processing`. The current stacked RECONCILE branch also guards known stale-sensitive direct storage-key lanes for Folder synthesis, Product signal/reading writes, and PR criteria/advanced-metrics writes. `TRACE` stays 🟡 until backend/direct LLM trace paths and a real full live artifact are locked; `RECONCILE` stays 🟡 because this is targeted lane coverage, not a repo-wide raw storage bypass rule.
- Recommended next slice after this stacked RECONCILE direct-key branch: land the full live backend/LLM trace work as a separate TRACE slice, preserving the full hover → save → queue → backend capture → direct LLM → Product analysis fixture gate.
