...
- C-Backend B1 landed in `dlens-ingest-core` PR #2 (`896373b`), extension B2 landed through PR #32-#34 (`23d36d1`), and backend B3 API typing landed in `dlens-ingest-core` PR #3 (`116e18c`) on 2026-06-15. `READMODEL_BACKEND` stays 🟡: backend duplicate-root, OP-continuation-chain, and `reply_edges` / `orphan_replies` are regression-tested; extension projection trusts that contract and preserves OP-reply/orphan metadata; API `thread_read_model` validates as `ThreadReadModel`. The remaining gate is expanded golden thread fixtures.
...
- C-Backend follow-up: keep `READMODEL_BACKEND` 🟡 until expanded golden thread fixtures are done. B2 projection alignment is merged: `src/contracts/ingest.ts` types `reply_edges` / `orphan_replies`, `src/state/captured-post.ts` stops same-author promotion when a backend read-model exists, and Product/Topic/SignalPacket evidence preserves OP-reply/orphan metadata without hiding visible replies. B3 backend API typing is also merged.
