...
- Product Action is a one-at-a-time stage over completed `try` / `watch` candidates, with text-labelled previous/next buttons, stage-owned keyboard navigation, and live page status. Four mutually exclusive verdict buckets own selection; the old macro scoreboard, pager dots, and multi-card wall are removed. `try` / `watch` render one persisted complete reading plus one copyable Agent brief; excluded and insufficient-data rows stay compact. Stage identity is keyed, direction is explicit, keyboard paging restores focus after keyed replacement, and selection follows signal identity when candidate order changes. Forward/backward x motion is token-owned under `no-preference`, and reduced motion clears animation, transition, and transform.
...
- Product Saved Signals is a compact lifecycle intake with no reading or export UI. Product Action owns four color-and-text verdict tiles, one keyed stage with a text pager, one complete persisted reading plus a copyable Agent brief for actionable verdicts, and one whole-folder packet-export shelf. Park/insufficient use the same stage geometry but show semantic exclusion/evidence-gap content only; do not reintroduce dots, a card wall, duplicate exclusion lanes, a Saved export surface, a review-action group, a selected-brief shelf, or the old `SignalReadingReviewWorkspace`.
...
- ProductSignalAnalyzer prompt/cache version is `v21`; strict provider output includes one optional `product_reading` and no longer asks for legacy proposal/deep-reading or recipe fields (`application_suggestions`, `watch_guidance`, `copy_recipe_markdown`, `workflow_stack`, `copyable_template`). Actionable analysis and its projected reading publish atomically; Product Action shows the complete grounded reading plus one Agent brief, not a second model call or a long tutorial recipe.
...
- Product SignalReading commits are cross-key writes: provider work stays outside the snapshot lock, then generation revalidates the live signal and saves the v21 analysis plus its projected reading atomically inside that lock. `signal/delete` therefore wins either linear order and cannot leave or restore an orphan Product reading.
...
- Product Agent Brief is derived only from the persisted, identity-matched reading for the signal currently mounted in the Action stage. The visible control copies that one brief; it does not create a second reading or review workflow.
...
## Version Rule As Of 2026-08-20

- Current source version: `0.3.59`, synchronized across all five lock sites. The manifest requests `unlimitedStorage`; Settings reports measured `chrome.storage.local` bytes as `X MB · 不限額` only after the background confirms that permission. This is a quota safety net for the long-lived collection library, not a substitute for removing full `latestCapture` mirrors or moving folder ownership to the backend.
- Fresh 0.3.59 integration gate: 1431 tests / 1426 pass / 5 skipped / 0 fail; typecheck, View/VM boundary guard, storage seam guard, migration fixtures, TRACE historical fixture harness, production build, bundle guard, and diff check pass. B-07 requires four verdict buckets, one text-paged Action stage, semantic park/insufficient content, and no card wall/dots. Actionable cards show one atomic, persisted complete reading plus a copyable Agent brief; there is no review-action group or selected-brief shelf. The built `DLens v3` manifest reports `0.3.59`; its manifest-referenced `output/chrome-mv3/content-scripts/threads.js` is 909329 raw / 257053 gzip -9 / 204670 brotli bytes within 918500 / 258700 / 206000 and has SHA-256 `50b31bd8bbee57cc8e6775301766b2f2447815fd58cef9ffd5c5e5b87d63522a`.
...
- Release `0.3.59` is published on `main` under `v0.3.59`. Static audit/test/build evidence does not itself make runtime green: real Chrome acceptance through the extension action or in-page launcher on a real Threads page in Jason's `Default` profile remains a separate runtime gate and requires healthy worker work truth. The load-unpacked path remains `output/chrome-mv3`.
...
- Repository/release state: use the Git tag and GitHub release for public version truth; do not reuse older local-only wording after a release is published.
...
- Backend request lifetime contract: JSON requests use a 30-second default timeout, `/health` uses 3 seconds, and Threads advanced metrics uses 120 seconds. Caller abort remains distinguishable from timeout, body consumption stays inside the timeout, and one request emits one terminal trace event.
- Topic audit write contract: evidence, memos, reports, episodes, cross-topic calibrations, publication, and clear operations use one recoverable module queue. Snapshot/global caches and broadcasts publish only after durable storage succeeds.
...
- Topic audit run contract: full runs persist typed request ownership with renewable 15-minute leases; every checkpoint and final publication re-checks the live owner with a fresh timestamp through the serialized mutation queue. Stale/expired owners cannot write, and successful owner-aware publication stores report + episodes while clearing the run entry atomically.
- Topic envelope contract: `empty`, `truncated`, and `schema_mismatch` are classified failures. A successful transport with an invalid envelope gets exactly one typed semantic retry (two semantic attempts maximum); transport retry remains separate. Google uses the keyless endpoint plus `x-goog-api-key`, and envelope telemetry carries no key, prompt, or raw model output.
- Topic source-session UI contract: persisted run truth hydrates into one priority-ordered state (generation, processing, crawl need, terminal failure, ready/current). One session card and one folder-scoped status rail own disclosure/progress; duplicate crawl/generation controls are removed. Review fixes preserve the previous Atlas across failed reruns, keep first-run/failure copy truthful, enforce accessible progress semantics, and align persisted report truth.
- Topic audit transient-state contract: mode/session changes reset reconciler ownership and local running flags immediately; late responses are ignored. Whole UI audit requests retain their 15-minute total guard.
