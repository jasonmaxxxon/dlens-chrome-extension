...
- Unreleased S7 candidate on `s7-polling-reconciliation`: `useExtensionSnapshot` stabilizes `sendAndSync`, the idle coordinator regression gate allows exactly five status requests across one minute, and the visible status copy now overrides stale `idle` with backend slow/unreachable truth. A real backend stop/restart moved the current Chrome health dot red → green, while the prior successful terminal refresh had already reconciled the real Topic to 7/7 and 126 comments. The active Chrome runtime was not reloaded after the latest build and still emitted 28 status requests in a 30-second sample, so do not call the live poll-rate gate green until the unpacked extension is explicitly reloaded. Keep 0.3.46 as the historical release; versioning remains pending the planned S6+S7 boundary.
- Current engineering branch: `main`. `release-0-3-46` (S2–S5) and the S7 polling/reconciliation work were merged to local main by 2026-07-15 (`e17945d`); S6 narrative semantic repair landed on main directly (`a796629`). Nothing is pushed to the GitHub remote yet.
