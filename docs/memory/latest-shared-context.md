...
## Version Rule As Of 2026-07-17

- Current source version: `0.3.49` (Topic session envelope resilience: owned/leased audit runs, typed envelope retry, Google header auth, and one source-session UI owner), synchronized across all five lock sites on 2026-07-17.
- Latest local verification for 0.3.49: 1289 total / 1284 pass / 0 fail / 5 skipped; typecheck, both boundary guards, storage seam guard, migration fixtures, fixture harness, production build, bundle guard, dual-manifest verification, and diff check passed. Final release bundle is 909846 raw / 255007 gzip -9 / 202909 brotli bytes, within unchanged limits 910000 / 256000 / 203000.
- Built `.output/chrome-mv3/manifest.json` and mirrored `output/chrome-mv3/manifest.json` both report `version: "0.3.49"` and `name: "DLens v3"`. The unpacked test path is `output/chrome-mv3`. Source/build 0.3.49 is static-ready, but Default Chrome reload/manual acceptance was not run because the owner explicitly required skipping Chrome/CDP; this static gate is not runtime proof and the release is not runtime-green.
