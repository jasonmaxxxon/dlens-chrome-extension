...
- C-Backend B1 landed in `dlens-ingest-core` PR #2 (`896373b`) on 2026-06-15. `READMODEL_BACKEND` is now 🟡: backend duplicate-root, OP-continuation-chain, and `reply_edges` / `orphan_replies` are regression-tested; extension B2 projection alignment is still pending.
...
- C-Backend B2: align extension projection with backend B1. `src/contracts/ingest.ts` must type `reply_edges` / `orphan_replies`; `src/state/captured-post.ts` must stop using same-author guessing to promote `discussion_replies` into OP continuations when a backend read-model exists; Product/Topic/SignalPacket evidence should preserve OP-reply/orphan metadata without hiding visible replies.
