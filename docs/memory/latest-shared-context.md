...
- Current source version: `0.3.48` (Wave 3 render hot-path hardening plus bounded backend requests, serialized Topic audit mutations, durable snapshot publication, and directly owned signal cleanup), synchronized across all five lock sites on 2026-07-17.
- Latest local verification for 0.3.48: 1242 tests / 1237 pass / 0 fail / 5 skipped; typecheck, both boundary guards, storage seam guard, migration fixtures, fixture harness, production build, bundle budget, and diff check passed.
- Built `.output/chrome-mv3/manifest.json` and mirrored `output/chrome-mv3/manifest.json` both report `version: "0.3.48"` and `name: "DLens v3"`. The unpacked test path is `output/chrome-mv3`. Chrome has not been reloaded for this release; manual runtime QA remains pending and the static gate must not be treated as live proof.
...
- Current engineering branch: `main`. The four reviewed 0.3.48 storage-integrity commits were fast-forward merged to local main on 2026-07-17; the release metadata and MV3 build are local only, with no push or tag and no Chrome reload yet.
