...
## Version Rule As Of 2026-08-24

- Current source version: `0.3.60`, synchronized across all five lock sites. The manifest requests `unlimitedStorage`; Settings reports measured `chrome.storage.local` bytes as `X MB · 不限額` only after the background confirms that permission. This is a quota safety net for the long-lived collection library, not a substitute for removing full `latestCapture` mirrors or moving folder ownership to the backend.
- Fresh 0.3.60 integration gate: all 151 test files pass; typecheck, View/VM boundary guard, storage seam guard, migration fixtures, production build, bundle guard, and diff check pass. Topic reopen now reconciles evidence only after complete Topic/session hydration, preserves the prior Atlas/memos/report/episodes as stale, and cannot overwrite an audit that has begun running. Failed-only sources remain retryable crawl work.
...
- Release `0.3.60` is published on `main` under `v0.3.60`. Real Chrome acceptance on 2026-08-24 used the in-page launcher on a real Threads page: the recovered backend reported reachable and idle, the current Topic inventory reconciled from 19 to 20, and one separate failed source surfaced as a retry target. No Atlas regeneration was invoked during this validation. The load-unpacked path remains `output/chrome-mv3`.
