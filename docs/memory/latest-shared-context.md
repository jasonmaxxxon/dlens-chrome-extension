...
- Release `0.3.59` is published on `main` under `v0.3.59`. Static audit/test/build evidence does not itself make runtime green: real Chrome acceptance through the extension action or in-page launcher on a real Threads page in Jason's `Default` profile is verified separately. The load-unpacked path remains `output/chrome-mv3`.
