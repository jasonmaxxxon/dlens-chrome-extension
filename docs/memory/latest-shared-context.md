...
- Product Action route stays on `ActionableInsightsBoard` only; it must not fall back to `SignalReadingReviewWorkspace` / `READING REVIEW` or render the Agent export / 原文優先 panel when stored readings exist.
