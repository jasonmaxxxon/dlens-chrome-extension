...
- C-Backend B1 landed in `dlens-ingest-core` PR #2 (`896373b`) and extension B2 landed through PR #32-#34 (`23d36d1`) on 2026-06-15. `READMODEL_BACKEND` stays 🟡: backend duplicate-root, OP-continuation-chain, and `reply_edges` / `orphan_replies` are regression-tested; extension projection now trusts that contract and preserves OP-reply/orphan metadata. The next gates are backend API typing and expanded golden thread fixtures.
