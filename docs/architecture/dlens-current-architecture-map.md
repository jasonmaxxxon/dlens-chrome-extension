...
> The mermaid above predates the topic-audit pipeline and does not yet carry these nodes; this textual node list is the live contract. Colours are honest to code as of 2026-07-12.
...
- **`TOPIC_AUDIT` 🟢** — multi-pass LLM (`src/state/topic-audit-handlers.ts`): P0.5 per-shard blank read → bounded P1 per-signal post synthesis → P2 lexicon → P3 narrative → P4 audience → P5 absence → P6 editor synthesis. P1 consumes bounded OP / OP-authored context / capture gaps plus persisted shard prose and structured hints under a hard 24,000-character ceiling; it never re-renders the raw audience pool. P6 consumes OP + cited-comment digest rather than all raw replies. Closed by the **local** deterministic `validateTopicAuditDraft`. ⚠️ P2-P6 still scale with the distilled signal set and do not share one global prompt-budget contract. ⚠️ **P7 LLM validator remains dead code** — `buildP7ValidatorPrompt` is defined + tested but is not called by the handler.
- **`TOPIC_AUDIT_INCREMENTAL` 🟢** — 0.3.35 separates per-signal content identity from positional evidence aliases. P0.5/P1 cache keys include signal content/reference hashes, exact provider:model, prompt/stage, shard policy, and upstream shard-set identity; append-only runs reuse unchanged signal reads and pay P0.5/P1 only for new/changed signals. Topic-level P2-P6 lenses intentionally rerun after an evidence-set change. Force runs receive a unique revision id even when fingerprints are unchanged.
- **`NARRATIVE_STATE` 🟢** — every successful P6 materializes a versioned, maximum-4096-character ledger of claims, voices, and open questions. Prior active claims must be explicitly confirmed/strengthened/weakened/retired and re-anchored to current evidence; handler-owned stable ids prevent the LLM from silently inventing continuity.
- **`TOPIC_EPISODES` 🟢** — the latest 24 first/advance/rebase episodes are persisted under `dlens:v1:topic-audit-episodes`. Same-fingerprint reruns replace the current episode while remapping display refs through stable anchors; report+episode publication is serialized and written together. ViewModel/UI accept report, memo, evidence, and episode only when revision identities agree, and Topic Detail renders the accepted episode as the `本次 / 自上次` delta strip.
...
- **`COMMENT_SHARD_READING` 🟢** — blank-read distillation layer between evidence-build and P1 is built: `CommentShardReading` folds into `TopicAuditMemoBundle` and now persists optional `reading` prose for legacy compatibility; the handler runs `comment-shard-reading` via the LLM seam. Full-audit and single-signal P1 both checkpoint missing shard reads before post synthesis. P1 consumes the bounded prose/structured distillate; P2/P4/P5 consume shard distillate + cited quotes instead of full raw packets.
- **`REACTION_PATTERN_MODEL` 🟢** — UI/parser/display contract for structured `ReactionCoverage` / `ReactionPattern` is built, and P4 is now the producer: it receives post-level shard-merged observations, uses real read-comment denominators, and stores `displayHints.reactionCoverage/reactionPatterns` after allowed-ref filtering. Still 🟢, not 🟩: LLM output quality is prompt/test guarded, but not a deterministic proof.
  Reaction counts may overlap because one comment can support more than one pattern. They are assignments, not a partition, so a donut/ring distribution would currently imply false additivity and is deliberately not rendered.
