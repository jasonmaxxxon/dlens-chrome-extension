...
| **Run 25 remaining QA + code fixes** (2026-06-11, Codex / Chrome `Default` = `jason@brandonproject.co`) | B-02 timing + Flow 8 + Flow 9–16 | **算作 pre-fix live 樣本 + code fix 輪；新版 live recheck pending**。B-02 in-page trace 成功捕捉：backend healthy 時 toggle response `509–1427ms`，content start/stop `16–792ms`，仍超 <300ms 目標；修掉 coordinator 因 `workerStatus` state change 自己重開 effect、連打 `/worker/status` 的問題（649/649 + typecheck + build）。Flow 8 **B-14**：Topic list 正常，但 `work` detail 顯示 `0 訊號 / 15/0 已分析 / 覆蓋 15/1`，修成 audit evidence/readings 作顯示分母並 sanitize invalid coverage。Flow 9–15 ✅：Saved/Classification/Action raw hits 0，Classification `技術討論 1 / 學習資源 1 / 噪音 1`，reading review file path 可收錄。Flow 16：HTML Reading export ✅（3 packets、檔案存在且無 raw residue）；Saved route batch copy 控制 missing → **B-15**，修成 saved route render `行動簡報匯出 / 原文優先 / 精簡決策 / 複製行動簡報`，Action route 仍不回歸舊 batch export。Chrome automation 明確禁止開 `chrome://extensions` 並要求不得 workaround，所以 reload `output/chrome-mv3` 待使用者手動後補 live recheck。Evidence：`docs/qa/assets/2026-06-11/run25/`。 |
| **Run 26 reload recheck + B-14 second fix** (2026-06-11, Codex / Chrome `Default` = `jason@brandonproject.co`) | B-02/B-14/B-15 reload recheck | **算作新版 live 樣本 + code fix 輪**。使用者手動 reload 後，Threads 頁面先出現舊 content script `Extension context invalidated`，reload Threads tab 後 marker 恢復。**B-02 ✅ live passed**：5× toggle response `308–489ms`、DOM wait `370–724ms`，content start/stop 多數 `1.5–2.6ms`（最慢一筆 108ms，overlap worker heartbeat），沒有 Run25 的 `925–1427ms` 自我重啟。**B-15 ✅ live passed**：Saved route 顯示 `行動簡報匯出 / 原文優先 / 精簡決策 / 複製行動簡報`，勾選後 `1 已選`、copy button enabled；scroll 到可視位置後 UI 顯示 `已複製`，macOS `pbpaste` 讀到 1190 字 structured Markdown，無 raw residue。**B-14 ❌ live recheck found second root cause**：list 仍 `work 1 訊號 0/1`，但 detail 變 `30 訊號 / 15/30 已分析 / 覆蓋 15/15`；原因是 `TopicDetailView` 仍把非 topic-scoped `signals.length` 膨脹到 audit overview。第二刀已修：有 audit evidence/memos 時 sourceTotal 只取 audit artifacts/coverage，stale queuedCount normalize；新增 regression。驗證 `110/110` targeted、typecheck、full `650/650`、build + mirror。**B-14 第二刀待使用者再 reload live recheck**。Evidence：`docs/qa/assets/2026-06-11/run26/`。 |
| **Run 27 B-14 live recheck** (2026-06-11, Codex / Chrome `Default` = `jason@brandonproject.co`) | Flow 8 / B-14 only | **算作新版 live 樣本，B-14 live passed**。使用者手動 reload `output/chrome-mv3` 後，先看到舊 content script `Extension context invalidated`，reload Threads page 後 marker 恢復。Topic → `work` detail 現在顯示 `15 訊號 / 15/15 已分析 / 覆蓋 15/15`，source rows `S1.OP–S15.OP`；負面檢查無 `30 訊號`、無 `15/30`。至此剩餘三項已收尾：B-02 Run26 passed、Flow 9–16 Run25/26 passed、Flow 8/B-14 Run27 passed。Evidence：`docs/qa/assets/2026-06-11/run27/b14-live-recheck.json`。 |

| **Run 28 signal drawer UI** (2026-06-11, Claude Code / 0.1.31) | 使用者 live 截圖檢視 SignalDrawer | **code 輪（無 live）**。使用者比對兩張 SignalDrawer 截圖提出兩件事。**inline 引用（診斷，非 bug）**：渲染器 `tokenizeProse` 本來就會把 prose 內 `[Sx.Ry]` 標記拆成 inline hover chip（截圖 2）；只有 prose 缺標記時才掉底部 `引用 →` fallback（截圖 1）。prompt 第 338 行強制 inline 標註是 `effb985`（2026-05-26）才加，work topic audit 早於此 → 那筆是舊 reading。**結論：re-analyze 該 signal 即修，零改 code**（待 live 驗）。**B-16 OP 卡片（code）**：`ThreadsStyleFragment` Threads 風格渲染早已存在但埋在預設收合的原文區；改成 OP root post 常駐頂部 Threads 卡片（avatar/@handle/date/ref/body/♥·💬），收合區改為留言串、無留言則隱藏。Full `652/652`、typecheck、build + mirror。Commit `03ce032`。**B-16 + inline 驗證待 live recheck**。 |

Open bugs（Run 28 後）：**無 Flow 1–16 open bug**；**B-16 OP 卡片 fixed in code（`03ce032`）待 live recheck**；inline 引用待 re-analyze 驗證（非 code bug）。Fixed / live passed：**B-01 · B-02 · B-03（idle 心跳 + recovery stamp 目視通過）· B-04（backend down + job last_error surface 皆有 live evidence；no-key 未重跑）· B-05（兩個 root cause：Run21 寫入防護 + Run22 orphan 清理誤刪，切換 ×3 + reload 通過）· B-06 · B-07 · B-08（主路徑 + HTML export）· B-09（panel save `已加入`）· B-10（detail time-link hover recheck 通過）· B-11（四 token 目視通過）· B-12（retrying job error 可見；manual drain 清除通過）· B-13（analysisError 不再誤標 Backend 離線）· B-14（Topic detail 15/15 count）· B-15（Saved route action brief copy）· Run19 launcher host**。Watch：backend down + inflight copy、Packet Export header count 口徑、B-04 no-key 分支仍可另開產品化 polish。
...
Run 25 evidence：`docs/qa/assets/2026-06-11/run25/`。核心檔：`b02-toggle-in-page-timing.json`（in-page trace pair，pre-fix healthy response `509–1427ms`）、`flow8-topic-count-contradiction.json`（Topic detail `0 訊號 / 15/0 / 覆蓋 15/1`）、`flow9-15-product-output-scan.json`（Saved/Classification/Action raw hits 0、reading filed）、`flow16-html-export-file.json`（HTML export file path + content checks）、`flow16-copy-missing-after-select.json`（Saved route selected checkbox 後 copy controls missing）。Regression：`tests/processing-state.test.ts` B-02 coordinator restart guard、`tests/topic-detail-view.test.tsx` B-14 denominator guard、`tests/views.test.tsx` B-15 saved batch export guard；驗證 `npm run typecheck` pass、full glob `649/649` pass、`npm run build` pass 並 mirror。Live recheck blocked until user reloads extension because Chrome automation policy blocks `chrome://extensions` and forbids workaround.
Run 26 evidence：`docs/qa/assets/2026-06-11/run26/remaining-qa-recheck.json`。核心結論：B-02 reload 後 live passed（response `308–489ms`）、B-15 live passed（`pbpaste` 1190 字 structured Markdown）、B-14 second root cause live confirmed（`30 訊號 / 15/30 / 覆蓋 15/15`）並 code fixed。Regression：`tests/topic-detail-view.test.tsx` 新增 non-topic-scoped `signals.length=30` guard；驗證 targeted `110/110`、typecheck、full glob `650/650`、`npm run build` pass 並 mirror。B-14 第二刀待 reload 後 live recheck。
Run 27 evidence：`docs/qa/assets/2026-06-11/run27/b14-live-recheck.json`。核心結論：reload 新 bundle + reload Threads page 後，Topic `work` detail live 顯示 `15 訊號 / 15/15 已分析 / 覆蓋 15/15`，無 `30 訊號` / `15/30`，B-14 第二刀 live passed。
...
| **B-02** | 1/4 | **P2** | 效能 | 開啟 collect mode 時同步遲滯 ~1–1.5s，Product/Topic workspace switch 偶爾 ~1s | 按下開啟 collect mode，觀察 panel/overlay 同步；切 Topic/Product | 預期 <300ms；Run25 pre-fix in-page trace：backend healthy toggle response `509–1427ms`、content start/stop `16–792ms`，仍超標；Run26 reload 後 response `308–489ms`、DOM wait `370–724ms`，未復現 1–2s | Codex Run 1/2 · `run25/b02-toggle-in-page-timing.json` · `run26/remaining-qa-recheck.json` | **fixed · live passed (Run26)** · Run21：toggle 改 tab-only storage write + cached mode read；Run25：`useProcessingCoordinator` 不再因 `workerStatus` state change 重開 effect/連打 `/worker/status`；Run26 live：content start/stop 即時，無 worker status self-restart。Watch：仍是 300–500ms 級，不是 hover 80ms 級 |
...
| **B-08** | 16 | **P2** | 匯出/IA | 已有 analyses 但 Review/Export workspace 被 existing `SignalReading` gate 擋住；first-reading action 存在但分散、不可發現 | Product `9 signals · 9 analyses` → Saved / Classification / Action / selected signal 搜尋 export/copy/reading controls | 預期：使用者可從分析結果清楚進入「生成 reading → review → export」；Run20 實際：`exportControls=[]`，點已分析 signal 後仍沒有 `深度判讀 / reading / Agent Brief / 匯出 / 複製` 控制；Run25 HTML Reading export 實際通過 | code `src/ui/ProductSignalViews.tsx:2208-2276,2570,2707,3781-3792,3889-3902` · Run2 UI buttons · Run5 code-path audit · `run20/flow9-16-live-evidence.json` · `run25/flow16-html-export-file.json` | fixed · **live passed 主路徑 + HTML export**：Run22 CTA → review workspace 接手；Run25 `匯出 HTML Reading` 產生 3-packet HTML 且 content checks 無 raw residue。Saved route batch copy 控制 missing 另歸 **B-15** |
...
| **B-14** | 8 | **P2** | 計數/Topic audit | Topic list 正常，但 Topic detail 使用 drifted / non-scoped signal sources 作分母，先顯示 `0 訊號 / 15/0 已分析 / 覆蓋 15/1`，Run26 又顯示 `30 訊號 / 15/30 已分析 / 覆蓋 15/15` | Topic workspace → open `work` topic detail after B-05 fixes | 預期：若 audit evidence/memos 有 15 source rows，detail header/overview 的可見分母應用 15，不可出現 `15/0`、`15/1` 或非 scoped `30 訊號`；Run27 live：`15 訊號 / 15/15 已分析 / 覆蓋 15/15`，無 `30 訊號`、無 `15/30` | `run25/flow8-topic-count-contradiction.json` · `run26/remaining-qa-recheck.json` · `run27/b14-live-recheck.json` · `tests/topic-detail-view.test.tsx` | **fixed · live passed (Run27)** · Run25 第一刀 sanitize impossible coverage；Run26 第二刀：有 audit artifacts 時 `TopicDetailView` 忽略非 scoped `signals.length`，並 normalize stale queuedCount；Run27 reload 後 confirmed |
| **B-15** | 16 | **P2** | 匯出/copy | Saved route 勾選 signal 後沒有 batch export/copy 控制；`SavedSignalsBatchExport` 元件存在但未 mount 到 saved-signals route | Product Saved → 勾選任一 signal → 找 `原文優先 / 精簡決策 / 複製行動簡報` | 預期：Saved route 手動選 signal 後可做行動簡報 copy；Run25 pre-fix 實際：checkbox checked 但 `hasBatchExport=false / hasCopyButton=false / hasModes=false`；Run26 reload 後 controls 出現，copy 寫入 system clipboard | `run25/flow16-copy-missing-after-select.json` · `run26/remaining-qa-recheck.json` · `tests/views.test.tsx` | **fixed · live passed (Run26)** · saved route render `行動簡報匯出`、兩種模式與 `複製行動簡報`；Action route 仍禁止舊 batch export。Run26 `pbpaste` 1190 字 structured Markdown，含原文/連結/觀眾反應/產品判斷，無 raw residue |
| **B-16** | signal drawer | **P2** | IA/原文呈現 | 點進 signal（SignalDrawer）只見 P1 判讀 + 收合的 `原文 · OP + 留言`；原文 thread 卡片要展開 toggle 才看得到，第一眼看不到原貼文 | Topic/Product → 開任一 signal drawer | 預期：點進 signal 即見模仿 Threads 設計的原文卡片；實際（Run28 前）：OP 內容藏在預設收合區 | 使用者 live 截圖 1/2 · `src/ui/SignalDrawer.tsx` · `tests/signal-drawer.test.tsx` | **fixed in code (`03ce032`)**：OP root post 常駐頂部 Threads 卡片（avatar/@handle/date/ref/body/♥·💬），收合區改為留言串、無留言隱藏，OP chip pinned 時卡片高亮。`652/652` + build。**live recheck pending** |
...
### 4.14 Run 25 Remaining QA + Code Fixes

Run 25 接在 Run24 後，使用 Jason profile 的同一 Threads detail tab `https://www.threads.com/@se_ranking/post/DZYf_mngRnt#dlensQaTrace=1`，private backend running。這輪先跑剩餘三塊：B-02 in-page timing、Flow 8 計數矛盾、Flow 9–16 export/copy + Classification 全掃。

| Item | 結論 | Evidence |
|------|------|----------|
| B-02 in-page timing | **pre-fix live failed，code fixed，待 reload recheck**。backend down 時 3× open/close response `369–500ms`；backend healthy 時 response `509–1427ms`，content start/stop `16–792ms`。trace 同時有 `popup.worker.status.*` 與 product hydrate overlap。修法：`useProcessingCoordinator` 改用 `lastKnownWorkerStatusRef`，`workerStatus` state change 不再重啟 polling effect，避免 idle heartbeat 在 toggle 周邊自我放大。 | `run25/b02-toggle-in-page-timing.json` · `tests/processing-state.test.ts` |
| Flow 8 count contradiction | **pre-fix live failed，code fixed，待 reload recheck**。Topic list `work/love` card 正常，但 open `work` detail 顯示 `0 訊號 / 15/0 已分析 / 覆蓋 15/1`，source rows `S1.OP–S15.OP` 仍可見。修法：Topic audit summary/overview 以 audit evidence/readings/sourceTotal 作分母，並 sanitize `N/0` 或 numerator > denominator 的 coverage。 | `run25/flow8-topic-count-contradiction.json` · `tests/topic-detail-view.test.tsx` |
| Flow 9–15 scan | **live passed**。Product Saved `3 signals · 3 analyses`；Classification 顯示 `技術討論 1 / 學習資源 1 / 噪音 1`；Action review/file path 可把 Fable reading 標成 `已收錄`，狀態變 `1 收錄 · 1/3 reviewed`。Saved/Classification/Action raw hits = 0。watch：Action verdict tile row 有 6px measured overflow，但未見嚴重破版；殘留英文 chrome（`Product mode`、`READING REVIEW`、`PACKET EXPORT` 等）仍屬 B-11 擴大範圍。 | `run25/flow9-15-product-output-scan.json` |
| Flow 16 HTML export | **live passed**。Action `匯出 HTML Reading` 後 UI status `exported`，filesystem 產生 `/Users/tung/Downloads/dlens-signal-packets-2026-06-11T05-52-55-074Z.html`，3 packets，檢查含三筆 source text 且無 `undefined/null/[object Object]/insufficient_data`。Playwright download event timeout，但 UI + file confirmed。 | `run25/flow16-html-export-file.json` |
| Flow 16 saved batch copy | **pre-fix live failed，code fixed，待 reload recheck**。Saved route 勾選 `signal_s2wfivlu_mq8w6z8g` 後 checkbox checked，但沒有 `原文優先 / 精簡決策 / 複製行動簡報`。root cause：`SavedSignalsBatchExport` 元件仍在，但 saved-signals route 沒 mount。修法：saved route 在有 analyses 時 render `行動簡報匯出`；Action route 仍保持 reading review / packet export，不回歸舊 batch export。 | `run25/flow16-copy-missing-after-select.json` · `tests/views.test.tsx` |

驗證：`npx tsx --test tests/processing-state.test.ts` pass；`npx tsx --test tests/views.test.tsx tests/topic-detail-view.test.tsx` pass；`npm run typecheck` pass；full glob `649/649` pass；`npm run build` pass 並 mirror 到 `output/chrome-mv3`。

Live recheck blocker：Chrome automation 嘗試打開 `chrome://extensions/` 時被 Browser Use policy 明確拒絕，且要求不能用 alternate browser surface / raw CDP / workaround 達成同一 reload 行為。因此 Run25 code fixes 尚未能在 Jason profile live recheck；需使用者手動 reload `output/chrome-mv3`。

Run 26 已執行；結果見 §4.15。

### 4.15 Run 26 Reload Recheck + B-14 Second Fix

Run 26 使用 Jason profile 同一 Threads detail tab。使用者手動 reload `output/chrome-mv3` 後，原 Threads page 上的舊 content script 先出現 `Extension context invalidated`；reload Threads tab 後新 content script 注入，QA trace 正常。

| Item | 結論 | Evidence |
|------|------|----------|
| B-02 timing | **live passed**。5× open/close：trace toggle response `308–489ms`、DOM wait `370–724ms`；content start/stop 多數 `1.5–2.6ms`，最慢一筆 108ms 且 overlap worker heartbeat。沒有 Run25 backend-healthy `925–1427ms` 的自我重啟退化。 | `run26/remaining-qa-recheck.json` |
| Flow 8 / B-14 | **live recheck failed with second root cause，code fixed，待 reload recheck**。Topic list 仍正確：`work/love = 1 訊號 0/1 已完成 0 處理中 1 待處理`；但 `work` detail 顯示 `30 訊號 / 15/30 已分析 / 覆蓋 15/15`，source rows `S1.OP–S15.OP`。Root cause：`TopicDetailView` 仍把非 topic-scoped `signals.length` 納入 audit overview denominator。第二刀：audit artifacts 存在時 sourceTotal 只取 evidence/memos/coverage；stale queuedCount normalize。 | `run26/remaining-qa-recheck.json` · `tests/topic-detail-view.test.tsx` |
| Flow 16 / B-15 copy | **live passed**。Saved route 出現 `行動簡報匯出 / 原文優先 / 精簡決策 / 複製行動簡報`；勾選後 `1 已選`、copy button enabled。將 popup scroll 到 copy button 可見後點擊，UI 顯示 `已複製`；`pbpaste` 讀到 1190 字 Markdown，包含原文、source URL、觀眾反應、產品判斷，無 `undefined/null/[object Object]`。 | `run26/remaining-qa-recheck.json` |

驗證：`npx tsx --test tests/views.test.tsx tests/topic-detail-view.test.tsx tests/processing-state.test.ts` → `110/110`；`npm run typecheck` pass；full glob `650/650` pass；`npm run build` pass 並 mirror 到 `output/chrome-mv3`。

### 4.16 Run 27 B-14 Live Recheck

Run 27 使用 Jason profile 同一 Threads detail tab。使用者手動 reload `output/chrome-mv3` 後，原 Threads page 先顯示舊 content script `Extension context invalidated`；reload Threads page 後新 content script 注入，root marker 仍為 `run14-url-trace-v1`。

| Item | 結論 | Evidence |
|------|------|----------|
| Flow 8 / B-14 | **live passed**。Topic → `work` detail 顯示 `15 訊號 / 15/15 已分析 / 覆蓋 15/15`，source rows `S1.OP–S15.OP`；負面檢查無 `30 訊號`、無 `15/30`。這補上 Run26 第二刀後唯一 pending 的 live gate。 | `run27/b14-live-recheck.json` |

Run 27 後剩餘三項收尾狀態：B-02 in-page timing 已於 Run26 live passed；Flow 9–16 export/copy + Classification 已於 Run25/26 live passed；Flow 8 計數矛盾已於 Run27 live passed。Flow 1–16 目前無 open bug。

Watch（非本輪必修）：

- backend down + inflight 時，文案仍可能同時說 `抓取正在進行` 與 `Backend 無法連線`；應改成「排隊中，等 backend 回來」這類狀態文案。
- Packet Export header 目前用 `signals.length`，會把未分析 / 未 reading 的 pending signal 算進 `3 signals → packet`；Run25 HTML export 內容可讀，但計數口徑仍可再產品化。

### 4.17 Run 28 Signal Drawer UI（inline 引用診斷 + B-16 OP 卡片）

使用者比對兩張 SignalDrawer live 截圖（work topic S1.OP 引用全在底部 vs ai-discussion S1.OP 引用 inline + hover），提出兩件事。逐件查 `src/ui/SignalDrawer.tsx`：

**① inline 引用 + hover —— 不是 renderer bug，是資料。**
- `tokenizeProse(reading.reading)` 用 `CITATION_GROUP_PATTERN` 掃 prose 內 `[Sx.Ry]` 方括號；有標記 → inline `CitationChip` + hover popover（截圖 2，預設行為）；`hasInlineRefs=false` 才掉到底部 `引用 →` fallback（截圖 1）。
- prompt 第 [338 行](../../src/compare/topic-audit-prompts.ts) 強制「prose 內 inline 標註 `[Sx.R1]`」是 commit `effb985`（2026-05-26, v0.1.22）才加；work topic 7-pass audit 在 2026-05-21 前後手跑，**早於此** → 那筆 reading 的 prose 沒有 inline 標記，落 fallback。
- **修法：re-analyze 該 signal（重生帶 inline 標記的 prose），零改 code。** 驗證：對一筆舊 work signal 重跑 P1 → drawer 應從底部清單變成 inline + hover（如截圖 2）。只有重 analyze 後**仍**掉底部（代表大輸入下模型不照規則）才需 prompt 硬化，另開 task。

**② B-16 OP Threads 卡片 —— code 已修（`03ce032`）。**
- `ThreadsStyleFragment`（Threads 風格列）早已存在，但只在預設收合的 `原文 · OP + 留言` toggle 內 render，點進 signal 第一眼看不到原貼文。
- 改：OP root post 抽到 body 頂部常駐 `OriginalPostCard`（avatar / @handle / date / `Sx.OP` ref / body / ♥·💬）；收合區改為「留言串（N 則）」只放 OP 接話 + 留言；無留言則整個 toggle 隱藏；OP 的 citation chip 被 pin 時頂部卡片高亮。
- 驗證：`signal-drawer.test.tsx` 加「OP 卡片常駐 + 留言收合」「無留言隱藏 toggle」兩條，full `652/652`、typecheck、build + mirror。

**Run 29 待辦（live，需 reload `output/chrome-mv3` ≥ `03ce032`）：**
1. **B-16 recheck**：開任一 signal drawer → 頂部即見 Threads 風格 OP 卡片（含原文 body）；展開「留言串」見 OP 接話 + 留言；只有 OP、無留言時 toggle 不出現。
2. **inline 引用驗證**：對一筆舊 work signal 按該行「分析此篇」re-analyze → drawer P1 prose 變成 inline chip + hover（截圖 2 樣）、底部 `引用 →` 消失。過了就確認此為舊資料、非 code 問題。
