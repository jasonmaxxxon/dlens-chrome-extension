...
Open bugs（Run 25 後）：**B-02 / B-14 / B-15 皆 fixed in code、待使用者 reload 後 live recheck**。Fixed / live passed：**B-01 · B-03（idle 心跳 + recovery stamp 目視通過）· B-04（backend down + job last_error surface 皆有 live evidence；no-key 未重跑）· B-05（兩個 root cause：Run21 寫入防護 + Run22 orphan 清理誤刪，切換 ×3 + reload 通過）· B-06 · B-07 · B-08（主路徑 + HTML export；saved batch copy 另歸 B-15）· B-09（panel save `已加入`）· B-10（detail time-link hover recheck 通過）· B-11（四 token 目視通過）· B-12（retrying job error 可見；manual drain 清除通過）· B-13（analysisError 不再誤標 Backend 離線）· Run19 launcher host**。仍需收尾：reload `output/chrome-mv3` 後重跑 B-02 timing、Flow 8 detail count、Flow 16 saved batch copy。
...
| **B-02** | 1/4 | **P2** | 效能 | 開啟 collect mode 時同步遲滯 ~1–1.5s，Product/Topic workspace switch 偶爾 ~1s | 按下開啟 collect mode，觀察 panel/overlay 同步；切 Topic/Product | 預期 <300ms；Run25 pre-fix in-page trace：backend healthy toggle response `509–1427ms`、content start/stop `16–792ms`，仍超標；trace 同時出現 worker/status 與 product hydrate overlap | Codex Run 1/2 · `run25/b02-toggle-in-page-timing.json` | fixed in code (candidate) · Run21：toggle 改 tab-only storage write + cached mode read；Run25：`useProcessingCoordinator` 不再因 `workerStatus` state change 重開 effect/連打 `/worker/status`（`tests/processing-state.test.ts`）。**待 reload 後 live recheck** |
...
| **B-14** | 8 | **P2** | 計數/Topic audit | Topic list 正常，但 Topic detail 使用 drifted topic signal pointers 作分母，顯示 `0 訊號 / 15/0 已分析 / 覆蓋 15/1` | Topic workspace → open `work` topic detail after B-05 fixes | 預期：若 audit evidence/memos 有 15 source rows，detail header/overview 的可見分母應用 15，不可出現 `15/0` 或 `15/1`；Run25 pre-fix 實際出現 invalid denominator | `run25/flow8-topic-count-contradiction.json` · `tests/topic-detail-view.test.tsx` | fixed in code · `TopicDetailView` / `useTopicAudit` 用 audit evidence/readings 作 sourceTotal 並 sanitize impossible coverage。**待 reload 後 live recheck** |
| **B-15** | 16 | **P2** | 匯出/copy | Saved route 勾選 signal 後沒有 batch export/copy 控制；`SavedSignalsBatchExport` 元件存在但未 mount 到 saved-signals route | Product Saved → 勾選任一 signal → 找 `原文優先 / 精簡決策 / 複製行動簡報` | 預期：Saved route 手動選 signal 後可做行動簡報 copy；Run25 pre-fix 實際：checkbox checked 但 `hasBatchExport=false / hasCopyButton=false / hasModes=false` | `run25/flow16-copy-missing-after-select.json` · `tests/views.test.tsx` | fixed in code · saved route 重新 render `行動簡報匯出`、兩種模式與 `複製行動簡報`；Action route 仍禁止舊 batch export。**待 reload 後 live recheck** |
...
Run 26 checklist（reload 後）：

1. **B-02**：Product Collect toggle open/close ×3，讀 in-page trace pair；目標 response + visible panel update <300ms，且不再看到連續 worker status 自我重啟。
2. **B-14**：Topic → open `work` detail；確認不再出現 `0 訊號 / 15/0 / 覆蓋 15/1`，應以 audit source total 顯示可理解分母。
3. **B-15**：Product Saved → 勾選 signal；確認 `行動簡報匯出 / 原文優先 / 精簡決策 / 複製行動簡報` 出現，點 copy 後 clipboard 是 structured text，無 `[object Object]` / API key / raw enum。
