...
Open bugs（Run 26 後）：**B-14 第二 root cause fixed in code、待使用者 reload 後 live recheck**。Fixed / live passed：**B-01 · B-02 · B-03（idle 心跳 + recovery stamp 目視通過）· B-04（backend down + job last_error surface 皆有 live evidence；no-key 未重跑）· B-05（兩個 root cause：Run21 寫入防護 + Run22 orphan 清理誤刪，切換 ×3 + reload 通過）· B-06 · B-07 · B-08（主路徑 + HTML export）· B-09（panel save `已加入`）· B-10（detail time-link hover recheck 通過）· B-11（四 token 目視通過）· B-12（retrying job error 可見；manual drain 清除通過）· B-13（analysisError 不再誤標 Backend 離線）· B-15（Saved route action brief copy）· Run19 launcher host**。仍需收尾：reload `output/chrome-mv3` 後重跑 Flow 8 `work` detail count。
...
| **B-14** | 8 | **P2** | 計數/Topic audit | Topic list 正常，但 Topic detail 使用 drifted / non-scoped signal sources 作分母，先顯示 `0 訊號 / 15/0 已分析 / 覆蓋 15/1`，Run26 又顯示 `30 訊號 / 15/30 已分析 / 覆蓋 15/15` | Topic workspace → open `work` topic detail after B-05 fixes | 預期：若 audit evidence/memos 有 15 source rows，detail header/overview 的可見分母應用 15，不可出現 `15/0`、`15/1` 或非 scoped `30 訊號`；Run26 live 證實第一刀只修 invalid denominator，未擋 `signals.length` 膨脹 | `run25/flow8-topic-count-contradiction.json` · `run26/remaining-qa-recheck.json` · `tests/topic-detail-view.test.tsx` | fixed in code · Run25 第一刀 sanitize impossible coverage；Run26 第二刀：有 audit artifacts 時 `TopicDetailView` 忽略非 scoped `signals.length`，並 normalize stale queuedCount。**待 reload 後 live recheck** |
...
Run 27 checklist（reload 後）：

1. **B-14 only**：手動 reload `output/chrome-mv3`，reload Threads page，Topic → open `work` detail；預期 header/overview 顯示 `15 訊號 / 15/15 已分析 / 覆蓋 15/15`，不再出現 `30 訊號 / 15/30`。
