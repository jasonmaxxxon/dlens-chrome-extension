...
2026-06-12 Slice 1 ✓ — added `src/state/pipeline-trace.ts` with the typed `PipelineEvent` contract, flag-gated typed emitter, DOM mirror, buffer cap, and validator; `src/ui/qa-trace.ts` is now only a typed adapter. Absorbed all production `markQaTrace("...")` call sites in `threads.content.ts`, `useInPageCollectorAppState.ts`, `useTopicState.ts`, and `useProcessingCoordinator.ts` into `emitPipelineEvent({ phase, step, target, result })` without rewriting hover extraction, storage transactions, polling/backoff, or UI behavior. Added characterization/boundary tests for flag-gating, structured entries, valid phases, explicit target objects, and grep-clean migrated call sites.
...
2026-06-12 Slice 3 ✓ — wired terminal `ui.ready` events from shell-owned ViewModel boundaries for Product, Topic, Compare, and PR Evidence. Added a UI-side `pipeline-ui-ready` adapter that maps VM `loadState` / `analysisState` / async brief state / PR resource state into `PipelineEventInput`, then emits through the existing flag-gated trace hook; VM builders remain pure and do not import trace, React, DOM, fetch, or Chrome APIs. Added characterization and boundary tests for all four surfaces and kept live QA harness / summarizer work out of this slice.

2026-06-12 Slice 4 ✓ — formalized `scripts/qa-trace-summary.mjs` around typed `PipelineEvent` traces while preserving legacy `{event}` trace artifact support. The summary now reports `phaseCounts`, first-error detection, first-seen phase journey, phase-to-phase latency, and a `--require-terminal ui.ready` gate that exits non-zero when the terminal phase is missing. Added `scripts/qa-live-pipeline-harness.mjs`, which can run `npm run build`, read a trace from a CDP live Threads tab or an already-dumped live trace file, write evidence JSON/Markdown, and assert the same terminal `ui.ready` gate. Added red/green CLI coverage for complete typed traces, missing-terminal failures, and harness evidence output. This slice does not rewrite hover extraction, storage transactions, polling/backoff, or VM builders; `TRACE` remains 🟡 until backend/direct LLM trace paths are integrated and a real live trace artifact passes the harness.

2026-06-12 Slice 5 ✓ — started `RECONCILE` by adding `src/state/request-reconcile.ts`: a pure lane + target + requestId reconciler that accepts only the latest current-target response and projects stale/target-mismatch ignores to a typed `ui.ready` trace event (`reconcile.stale-result.ignore`). Wired it into Compare result async fetches (brief / cluster summaries / evidence annotations), Product analyze + signal-reading actions, Folder synthesis generate/clear, PR Evidence criteria/match/metrics/summary actions, and a narrow session-scoped `sendAndSync` snapshot guard for long async writes. Added pure red/green tests plus controller/source boundary coverage. `RECONCILE` moves 🔴→🟡, not 🟩, because background/storage seam-wide stale write rejection is still pending.

2026-06-13 Slice 6 ✓ — expands `TRACE` for the full live path: adds `backend.request` and `llm.call` phases, instruments ingest backend HTTP calls and direct provider HTTP calls, mirrors service-worker trace events back into the active QA page when `dlensQaTrace=1` is enabled, and adds `--require-phases` to the trace summary/live harness. Jason-profile live QA captured `docs/qa/assets/2026-06-13/full-live-backend-llm/live-trace-full-hover-save-queue-analysis.json` (hover → save → queue → backend capture → direct Google LLM → Product analysis, 900 events, no pipeline errors), with harness/summary outputs beside it. The committed full-phase gate is now the CI fixture, so `TRACE` moves to 🟩 when PR #29 lands.
