...
async function runAuditPipeline(
  ...
  const signalReadings: SignalReading[] = fromStage && reusableMemos && resumeIndex > 0
  ...
  if (signalReadings.length === 0) {
    ...
    await saveMemos(storageArea, topic.id, auditRunId, inputHash, signalReadings, []);
  }
