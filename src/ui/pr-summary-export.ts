function sanitizeFileBase(name: string): string {
  return (name || "pr-evidence-summary")
    .trim()
    .replace(/[\\/:*?"<>|]+/g, "-")
    .replace(/\s+/g, "-")
    .slice(0, 80) || "pr-evidence-summary";
}
...
export function exportPrSummaryMarkdown(summary: string, campaignName: string): void {
  downloadBlob(
    new Blob([summary], { type: "text/markdown;charset=utf-8" }),
    `${sanitizeFileBase(campaignName)}-summary.md`
  );
}
...
export function buildPrSummaryDocx(summary: string): Blob {
...
export function exportPrSummaryDocx(summary: string, campaignName: string): void {
  downloadBlob(buildPrSummaryDocx(summary), `${sanitizeFileBase(campaignName)}-summary.docx`);
}
