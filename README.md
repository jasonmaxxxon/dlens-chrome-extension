...
- Product Action route stays on the candidate-action board even when `SignalReading` rows already exist; the old `READING REVIEW` / `PACKET EXPORT` workspace and Agent export panel are no longer the action page.
