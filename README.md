...
the verified `main` build. Current extension version is `0.1.12`. The source checkout may contain local
...
- **Version lock (2026-05-18)**: extension version is `0.1.12` across `package.json`, `package-lock.json`, `wxt.config.ts` manifest version, and `src/ui/version.ts`. Chrome's extension page reads the built manifest version; the popup masthead reads `BUILD_VERSION`.
