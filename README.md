...
the verified `main` build. Current extension version is `0.1.14`. The source checkout may contain local
...
- **Motion Layer v1 (2026-05-19)**: shared pure-CSS/token motion now covers buttons, rail icons, card hover/lift, smooth disclosure, and Product reading-review feedback. Content-script CSS is scoped under `data-dlens-control="true"` and respects `prefers-reduced-motion`.
- **Version lock (2026-05-19)**: extension version is `0.1.14` across `package.json`, `package-lock.json`, `wxt.config.ts` manifest version, and `src/ui/version.ts`. Chrome's extension page reads the built manifest version; the popup masthead reads `BUILD_VERSION`.
