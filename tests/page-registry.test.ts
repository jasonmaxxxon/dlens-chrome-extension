...
test("page registry owns the current allowed-page contract", () => {
  ...
  assert.deepEqual(getAllowedPagesForMode("product"), ["signals", "collect"]);
...
test("page registry derives home pages and rail-visible pages", () => {
  ...
  assert.equal(getHomePageForMode("product"), "signals");
  ...
  assert.deepEqual(getRailPagesForMode("product"), ["signals", "collect"]);
...
test("page registry classifies component families and widths", () => {
  ...
  assert.equal(getPageComponentKind("signals"), "product-signal");
  ...
  assert.equal(isPageComponentKind("signals", "product-signal"), true);
...
test("popup consumes page registry instead of re-declaring page families", () => {
  ...
  assert.doesNotMatch(source, /guardedPage === "signals"\s*\|\|/);
